...
IF ($action =="delete"){ // ***************** DELETE the record ***************************
    ...
    $delSQL = "DELETE FROM sk_groups_modules_link  where groupid = '$saField[groupid]'";
        if ($GLOBALS["debug"]>0) $Messages .= $delSQL;
        $result=$skdbadm->Execute($delSQL);
        if ($result == false) $Messages .= " DB-DELETE failed! ".$skdbadm->Errormsg()."<br>";
    $delSQL = "DELETE FROM sk_groups_users_link  where groupid = '$saField[groupid]'";
        if ($GLOBALS["debug"]>0) $Messages .= $delSQL;
        $result=$skdbadm->Execute($delSQL);
        if ($result == false) $Messages .= " DB-DELETE failed! ".$skdbadm->Errormsg()."<br>";
...
IF (empty($saField[groupid])) $saField[groupid]=-1;
$sql = "SELECT * FROM $table  where groupid = '$saField[groupid]'"; //SELECT a record
$rs = $skdb->Execute($sql);

    IF ($saField[groupid]==-1): // *********** INSERT *************
        unset($saField[groupid]);
    ...
    ELSE:                   // *********** UPDATE *************
        ...
        if ($result == false AND $skdbadm->Errormsg()<>"Query was empty"): $Messages .= "DB-update failed! ".$skdbadm->Errormsg()."<br>";endif;
        ...
        if (empty($moduleAdmin)) $moduleAdmin=array();
        foreach($moduleAdmin as $admin_right_module){
            $insertSQL = "INSERT INTO sk_groups_modules_link (groupid, mid, type) VALUES ($saField[groupid], $admin_right_module, 'A')";
            $result = $skdbadm->Execute($insertSQL);
            if ($GLOBALS["debug"]>0) $Messages .= $insertSQL;
        }
...
<? while (!$result->EOF AND !$result === false) {?>
  <tr height="25" onmouseover="this.style.backgroundColor = '#ffffff'" onmouseout="this.style.backgroundColor = '#eeeeee'">
    ...
    <a href="sk_groups_edit.php?current_page=<? echo $result->AbsolutePage();?>&saField[groupid]=<? echo $result->fields[groupid];?>&alvis=<?=$alvis?>"><?echo $GLOBALS['edit_button'];?> </a>
    <?echo "<a href=\"javascript:void(0);\" OnClick=\"frage_s('saFieldid',".$result->fields[groupid].");return false\"><img src=\"".SKRES_URL."skinz/".$GLOBALS['theme']."/buttons/del_icon.gif\"  border=\"0\" align=\"baseline\"></a>";?>
    </td>
  </tr>
   <tr>
...
<tr>
      ...
      <b><a href="sk_groups_edit.php">neue Gruppe anlegen <?echo $GLOBALS['add_button'];?></a></b>
      </td>
      </tr>
    </table>
