...
// Ver:		$Id: jpgraph.php,v 1.143 2002/10/30 10:25:50 aditus Exp $
//
...
DEFINE("TTF_DIR","/opt/jdk1.2.2/jre/lib/fonts/");
...
// PRIVATE METHODS	
	...
	switch( $this->background_image_type ) {
		...
		//echo "width=".$this->img->width.", height=".$this->img->height."<p>";
		$GLOBALS['copyfunc']($this->img->img,$bkgimg,
				     0,0,0,0,
	...
	// Stroke title
	$y = $this->title->margin; $margin=3;
	$this->title->Center(0,$this->img->width,$y);
	$this->title->Stroke($this->img);
		
	// ... and subtitle
	$y += $this->title->GetFontHeight($this->img) + $margin + $this->subtitle->margin;
	$this->subtitle->Center(0,$this->img->width,$y);	
	$this->subtitle->Stroke($this->img);

	// ... and subsubtitle
	$y += $this->subtitle->GetFontHeight($this->img) + $margin + $this->subsubtitle->margin;
...
//	echo "min=$min, max=$max, majstep=$majstep, minstep=$minstep, adjmax=$adjmax, adjmin=$adjmin<br>";
...
class Image {
	...
	if( $GLOBALS['gd2']==true && USE_TRUECOLOR ) {
	    ...
	    if( $this->img < 1 ) {
		die("<font color=red><b>JpGraph Error:</b></font> Can't create image. Check that you really have correct GD library installed.");
	    }
	}		
	assert($this->img > 0);		
...
DEFINE(\"USE_APPROX_COLORS\",true);
	...
	$this->FilledArc($xt+$r+1,$yt+$r+1,$r*2,$r*2,180,270);
	$this->FilledArc($xr-$r-1,$yt+$r+1,$r*2,$r*2,270,360);

	// Bottomleft & Bottom right arc
	$this->FilledArc($xt+$r+1,$yl-$r-1,$r*2,$r*2,90,180);
	$this->FilledArc($xr-$r-1,$yl-$r-1,$r*2,$r*2,0,90);
