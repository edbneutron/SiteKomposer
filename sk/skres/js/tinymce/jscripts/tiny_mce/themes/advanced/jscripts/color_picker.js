...
function renderColorMap() {
	...
	for (var i=0; i<colors.length; i++) {
		html += '<td bgcolor="' + colors[i] + '">'
			...
			+ '<img border="0" src="index.php?action=resfile&filename=js/tinymce/jscripts/tiny_mce/themes/advanced/images/spacer.gif" width="10" height="10" title="' + colors[i] +  '" alt="' + colors[i] +  '" /></a></td>';
		if ((i+1) % 18 == 0)
			html += '</tr><tr>';
	}
	html += '<tr><td colspan="18">'
		...
		+ '<tr><td>'
		+ '<img id="selectedColor" style="background-color:' + tinyMCE.getWindowArg('input_color') + '" border="0" src="index.php?action=resfile&filename=js/tinymce/jscripts/tiny_mce/themes/advanced/images/spacer.gif" width="80" height="16" />'
		+ '</td><td align="right">'
