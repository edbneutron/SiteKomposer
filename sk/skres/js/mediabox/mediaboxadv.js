...
*/
		...
		/*
		 * Resize function added by Edgar Bueltemeyer
		 */
		resize: function(newWidth,newHeight){ 
			
			top = window.getScrollTop() + (window.getHeight()/2);
			left = window.getScrollLeft() + (window.getWidth()/2);
			/* too small browser window */
			if (window.getHeight() < newHeight) {
				newHeight = window.getHeight() - 60;
				//top = top-40;
			}
			
			fx.resize = new Fx.Morph(center, $extend({duration: options.resizeDuration, onComplete: imageAnimate}, options.resizeTransition ? {transition: options.resizeTransition} : {}));
			fx.resize.start({height: newHeight+60, width: newWidth+20, marginTop: -top, marginLeft: -newWidth/2});
			
			if(mediaType == 'url'){
				image.setStyle("width",newWidth);
				image.setStyle("height",newHeight);
				$(mediaId).width=newWidth;
				$(mediaId).height=newHeight;
