...
(function($){
	...
	//Variables
	var interfaceHeight, interfaceWidth, loadedHeight, loadedWidth, element, index, settings, open, callback, colorbox = 'colorbox', hover = 'hover';

	//Functions
	var prev, next, init, load, position, dimensions, slideshow, close;
	
	//Events
	var cbox_open = 'cbox_open', cbox_load = 'cbox_load', cbox_complete = 'cbox_complete', cbox_close = 'cbox_close', cbox_closed = 'cbox_closed';
	...
	position = function(mWidth, mHeight, speed, loadedCallback){
		...
		var animate_speed = ($cbox.width()===mWidth && $cbox.height() === mHeight) ? 0 : speed;
	...
	dimensions = function(object){
		...
		var photo = $('#cboxPhoto')[0];
		if(photo && settings.height){
			var topMargin = (height - parseInt(photo.style.height, 10))/2;
		...
		if(settings.preloading && $related.length>1){
			...
			var nextSrc = $(next).data(colorbox).href || next.href;
			var prevSrc = $(prev).data(colorbox).href || prev.href;
	...
	load = function(){
		...
		var href = settings.href || element.href;
	...
	close = function(){
		...
		$window.unbind('resize.cbox_resize resize.cboxie6 scroll.cboxie6');//EDIT TEST
	...
	method.element = function(){ return element; };
