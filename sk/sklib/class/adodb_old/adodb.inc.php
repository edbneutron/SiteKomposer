...
/** 
 * @version V1.40 19 September 2001 (c) 2000, 2001 John Lim (jlim@natsoft.com.my). All rights reserved.
...
*/
 ...
 if (!defined('_ADODB_LAYER')) {
	 ...
	 */
	$ADODB_vers = 'V1.40 19 September 2001 (c) 2000, 2001 John Lim (jlim@natsoft.com.my). All rights reserved. Released LGPL.';
	...
	function Connect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "") {
		...
		$rez = $this->_connect($argHostname,$argUsername,$argPassword,$argDatabaseName);
		if (!$rez &&$this->debug) print $argHostname.': '.$this->ErrorMsg().'<br>';
		return $rez;
	}	
	...
	function PConnect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "")
	{
		...
		if ($this->debug) print $argHostname.': '.$this->ErrorMsg().'<br>';
		return false;
	}
	...
	*/
	...
	$nofixquotes=false;
		if (!$magic_quotes) {
			if ($this->replaceQuote[0] == '\\') $s = str_replace('\\','\\\\',$s);
...
$curr_page = $HTTP_GET_VARS['current_page'];
$rs = $conn->PageExecute($sql, $num_of_rows_per_page, $curr_page);
...
//----------------------------------------------
//And then, for the "Previous" and "Next" links:

...
<?php
if (!rs->AtFirstPage() || !rs->AtLastPage()) {
	if (!rs->atFirstPage()) {
?>
<a href="....?current_page=<?php echo $rs->AbsolutePage() - 1 ?>">Previous page</a>
<?php
	}
	if (!rs->atLastPage()) {
?>
<a href="....?current_page=<?php echo $rs->AbsolutePage() + 1 ?>">Next page</a>
<?php
	}
}
?>
