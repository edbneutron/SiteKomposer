...
V1.40 19 September 2001 (c) 2000, 2001 John Lim (jlim@natsoft.com.my). All rights reserved.
  ...
  the BSD license will take precedence.
	Made table name configurable - by David Johnson djohnson@inpro.net
 	Added encrypted session support - Ari Kuorikoski <kuoriari@finebyte.com>

  WARNING: The $ADODB_CRYPT_KEY is visible, so this encryption is mostly useful
       to prevent casual users from peeking/hacking into the session info.
...
include_once(dirname(__FILE__).'/crypt.inc.php');
...
if (empty($ADODB_SESSION_DRIVER)) {
	...
	$ADODB_SESSION_USER ='PHP_SESSIONS';
	$ADODB_SESSION_PWD ='session_user';
...
{
$Crypt = new MD5Crypt;
global $ADODB_SESS_CONN,$ADODB_SESS_INSERT,$ADODB_SESSION_TBL,$ADODB_CRYPT_KEY;
	...
	if ($rs) {
		if ($rs->EOF) {
			...
			$v = rawurldecode($Crypt->Decrypt($rs->fields[0], $ADODB_CRYPT_KEY));
		}
		$rs->Close();
		return $v;
	}
...
{
$Crypt = new MD5Crypt;
	global $ADODB_SESS_INSERT,$ADODB_SESS_CONN, $ADODB_SESS_LIFE, $ADODB_SESSION_TBL, $ADODB_CRYPT_KEY;

	$expiry = time() + $ADODB_SESS_LIFE;

	// encrypt session data..	

	$val = $Crypt->Encrypt(rawurlencode($val), $ADODB_CRYPT_KEY);
